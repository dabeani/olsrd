...
function resolveIPv6Hostname(ipv6, callback) {
...
function populateOlsr2Tab(data) {
  ...
  // Only populate if the OLSR2 tab is currently active
  var tabOlsr2 = document.getElementById('tab-olsr2');
  if (!tabOlsr2 || !tabOlsr2.classList.contains('active')) {
    return; // Don't populate if tab is not active
  }
  
  // Ensure we're appending to the correct tab - check if olsr2-content is inside tab-olsr2
  if (!tabOlsr2.contains(content)) {
