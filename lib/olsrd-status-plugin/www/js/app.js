...
function updateUI(data) {
  ...
  try {
    ...
    if (statusTab) {
      ...
      var dynamicItems = container.querySelectorAll('.info-item.dynamic');
      dynamicItems.forEach(function(item) { item.remove(); });
      // Add OLSRd Version
      var olsrdVersionItem = document.createElement('div');
      olsrdVersionItem.className = 'info-item dynamic';
      olsrdVersionItem.innerHTML = '<div class="info-label"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> OLSRd Version</div><div class="info-value">' + (data.versions && data.versions.versions_string ? data.versions.versions_string : 'unknown') + '</div>';
      container.appendChild(olsrdVersionItem);
      ...
      if (data.olsr2_on && data.olsr2_version) {
        // OLSR2 Version
        var olsr2VersionItem = document.createElement('div');
        olsr2VersionItem.className = 'info-item dynamic';
        var version = data.olsr2_version && data.olsr2_version.version && data.olsr2_version.version[0];
        olsr2VersionItem.innerHTML = '<div class="info-label"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> OLSR2 Version</div><div class="info-value">' + (version ? version.version_text || 'unknown' : 'unknown') + ' | ' + (version ? version.version_commit || 'unknown' : 'unknown') + '</div>';
        container.appendChild(olsr2VersionItem);
        // OLSR2 Uptime
        var olsr2UptimeItem = document.createElement('div');
        olsr2UptimeItem.className = 'info-item dynamic';
        var timeData = data.olsr2_time && data.olsr2_time.time && data.olsr2_time.time[0];
        var uptime = timeData ? timeData.time_internal || 'unknown' : 'unknown';
        var uptimeStr = timeData ? timeData.time_system || 'unknown' : 'unknown';
        olsr2UptimeItem.innerHTML = '<div class="info-label"><span class="glyphicon glyphicon-time" aria-hidden="true"></span> OLSR2 Uptime</div><div class="info-value">' + uptimeStr.split('.')[0] + ' | ' + uptime + '</div>';
        container.appendChild(olsr2UptimeItem);
        // OLSR2 Originator
        var olsr2OriginatorItem = document.createElement('div');
        olsr2OriginatorItem.className = 'info-item dynamic';
        var originator = data.olsr2_originator && data.olsr2_originator.originator && data.olsr2_originator.originator[1];
        olsr2OriginatorItem.innerHTML = '<div class="info-label"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> Originator</div><div class="info-value">' + (originator ? originator.originator || 'unknown' : 'unknown') + '</div>';
        container.appendChild(olsr2OriginatorItem);
        // IPv6 Default Route
        var ipv6RouteItem = document.createElement('div');
        ipv6RouteItem.className = 'info-item dynamic';
        ...
        if (ipv6Route && ipv6Route.ip && ipv6Route.ip.indexOf(':') !== -1) {
          var parts = [];
          if (ipv6Route.hostname) parts.push('<a target="_blank" href="https://[' + ipv6Route.ip + ']">' + ipv6Route.hostname + '</a>');
          else parts.push('<a target="_blank" href="https://[' + ipv6Route.ip + ']">' + ipv6Route.ip + '</a>');
          if (ipv6Route.dev) parts.push(' via ' + ipv6Route.dev);
          ipv6Html = parts.join(' ');
        }
        ipv6RouteItem.innerHTML = '<div class="info-label"><span class="glyphicon glyphicon-road" aria-hidden="true"></span> IPv6 Default Route</div><div class="info-value">' + ipv6Html + '</div>';
        container.appendChild(ipv6RouteItem);
      }
    }
  } catch(e) {}
