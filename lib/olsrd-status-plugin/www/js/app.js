    ...
    try {
      ...
      if (id === '#tab-status') {
        ...
        try {
          ...
          // Start polling live links while status tab is active (respect stored preference)
          try {
            var _lse = '1'; try { _lse = localStorage.getItem('links_live_enabled'); } catch(e) {}
            if (_lse === null || _lse === '1' || _lse === 'true') {
              try { startLinksPoll(); } catch(e) { console.warn('startLinksPoll failed', e); }
            }
          } catch(e) { console.warn('links poll start check failed', e); }
