...
document.addEventListener('DOMContentLoaded', function(){
  try {
    var diagBtn = document.getElementById('footer-diagnostics');
    ...
    if (diagBtn && modal && closeBtn && content) {
      diagBtn.addEventListener('click', function(){ modal.style.display='block'; renderDiagPage('intervals'); });
      closeBtn.addEventListener('click', function(){ modal.style.display='none'; });
      document.querySelectorAll('.diag-nav').forEach(function(b){ b.addEventListener('click', function(){ var p = b.getAttribute('data-page'); renderDiagPage(p); }); });
    }
    function renderDiagPage(page){
      try {
        ...
        if (page === 'params') {
          ...
          var t = document.createElement('pre'); t.style.whiteSpace='pre-wrap'; t.textContent = JSON.stringify({ fetch_auto_refresh_ms: window._fetch_auto_interval_ms || null, links_poll_interval_ms: window._links_poll_interval_ms || null }, null, 2);
          ...
          var lbl = document.createElement('div'); lbl.textContent='Polling intervals (seconds) — change and they take effect immediately'; row.appendChild(lbl);
          function addRow(key, localKey, defaultSec) {
            var wr = document.createElement('div'); wr.style.display='flex'; wr.style.alignItems='center'; wr.style.gap='8px';
            var kLbl = document.createElement('div'); kLbl.style.width='220px'; kLbl.textContent = key; wr.appendChild(kLbl);
            var inp = document.createElement('input'); inp.type='number'; inp.min='1'; inp.style.width='80px'; inp.value = (localStorage.getItem(localKey) || String(defaultSec));
            inp.addEventListener('change', function(){ var v = parseInt(inp.value,10)||defaultSec; localStorage.setItem(localKey,String(v)); if (localKey === 'links_live_interval_s') { window._links_poll_interval_ms = v*1000; try{ if (window._links_poll_handle) { stopLinksPoll(); startLinksPoll(); } }catch(e){} } });
            wr.appendChild(inp);
            row.appendChild(wr);
          }
          addRow('Links live interval', 'links_live_interval_s', Math.max(1, Math.floor(window._links_poll_interval_ms/1000) || 10));
          addRow('Status auto-refresh (ms)', 'fetch_auto_refresh_ms', window._fetch_auto_refresh_ms || 10000);
          ...
          logs.slice().reverse().forEach(function(l){
            var r = document.createElement('div'); r.style.border='1px solid #eee'; r.style.padding='6px'; r.style.cursor='pointer';
            r.textContent = '['+ (l.status || l.error || '-') +'] ' + l.method + ' ' + l.url + ' ('+(l.duration_ms||'-')+'ms)';
            r.addEventListener('click', function(){
              var d = document.createElement('pre'); d.style.whiteSpace='pre-wrap'; d.textContent = JSON.stringify(l, null, 2);
              content.innerHTML = ''; content.appendChild(d);
            });
            tbl.appendChild(r);
          });
