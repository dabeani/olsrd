  ...
  try { if (window._uiDebug) { var pane = document.querySelector(id); if (pane) { var cs = window.getComputedStyle(pane); console.debug('onTabShown: id=', id, 'classes=', pane.className, 'computedDisplay=', cs.display, 'visible=', cs.display!=='none'); } } } catch(e){}
    ...
    try {
      ...
      if (id === '#tab-olsr') {
        ...
        try {
          // Add OLSR2 links function
          function addOlsr2Links(st) {
            if (!st.olsr2_on) return;
            ...
            // populate with OLSR2 links
            fetch('/olsr2', {cache:'no-store'}).then(function(r){ return r.json(); }).then(function(olsr2data){
              try {
                if (olsr2data.links && Array.isArray(olsr2data.links)) {
