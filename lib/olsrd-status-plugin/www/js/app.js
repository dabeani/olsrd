...
function updateUI(data) {
  ...
  try {
    ...
    if (statusTab) {
      ...
      // OLSRd Version
      var dt1 = document.createElement('dt');
      dt1.innerHTML = 'Olsrd Version <span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>';
      var dd1 = document.createElement('dd');
      var version = data.versions && data.versions.versions_string ? data.versions.versions_string : 'unknown';
      dd1.textContent = version;
      dl.appendChild(dt1);
      dl.appendChild(dd1);
      // System Uptime
      var dt2 = document.createElement('dt');
      dt2.innerHTML = 'System Uptime <span class="glyphicon glyphicon-time" aria-hidden="true"></span>';
      var dd2 = document.createElement('dd');
      var uptime = data.uptime_linux || data.uptime_str || data.uptime || 'unknown';
      dd2.textContent = uptime;
      dl.appendChild(dt2);
      dl.appendChild(dd2);
      // IPv4 Default Route
      var dt3 = document.createElement('dt');
      dt3.innerHTML = 'IPv4 Default Route <span class="glyphicon glyphicon-road" aria-hidden="true"></span>';
      var dd3 = document.createElement('dd');
      var route = data.default_route;
      if (route && (route.ip || route.dev || route.hostname)) {
        var host = route.hostname || '';
        var ip = route.ip || '';
        var dev = route.dev || '';
        var parts = [];
        if (host) parts.push('<a target="_blank" href="https://' + host + '">' + host + '</a>');
      ...
      dl.appendChild(dt3);
      dl.appendChild(dd3);
      // Insert before OLSRv2 dl if present
      var olsr2Dl = statusTab.querySelector('dl.dl-horizontal');
      if (olsr2Dl) {
        statusTab.insertBefore(dl, olsr2Dl);
      } else {
        statusTab.appendChild(dl);
      }
      // Remove the old versions-container
      var container = statusTab.querySelector('.versions-container');
      if (container) container.remove();
    }
  } catch(e) {}
  // Add OLSRv2 info to status tab
  try {
    if (data.olsr2_on && data.olsr2_version) {
      ...
      if (statusTab) {
        ...
        // IPv6 Default Route
        var dt4 = document.createElement('dt');
        dt4.innerHTML = 'IPv6 Default Route <span class="glyphicon glyphicon-road" aria-hidden="true"></span>';
        var dd4 = document.createElement('dd');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function showTab(targetId) {
    ...
    // Clean up any misplaced OLSRv2 content that might be in the wrong tab
    try {
      var olsr2Content = document.getElementById('olsr2-content');
      var olsr2Tab = document.getElementById('tab-olsr2');
      if (olsr2Content && olsr2Tab && !olsr2Tab.contains(olsr2Content)) {
        // Move olsr2-content back to its proper location
        var h3 = olsr2Tab.querySelector('h3');
        if (h3) {
          h3.parentNode.insertBefore(olsr2Content, h3.nextSibling);
        } else {
          olsr2Tab.appendChild(olsr2Content);
        }
      }
    } catch(e) {}
  ...
  try { if (window._uiDebug) { var pane = document.querySelector(id); if (pane) { var cs = window.getComputedStyle(pane); console.debug('onTabShown: id=', id, 'classes=', pane.className, 'computedDisplay=', cs.display, 'visible=', cs.display!=='none'); } } } catch(e){}
    ...
    try {
      ...
      if (id === '#tab-olsr2') {
        console.log('Loading OLSR2 links tab content...');
        setTabLoading('tab-olsr2');
        try {
          // Fetch status to populate OLSR2 data
          safeFetchStatus({cache:'no-store'}).then(function(st){
            try {
              if (st && st.olsr2_on) {
                populateOlsr2Tab(st);
              }
              setTabLoaded('tab-olsr2');
            } catch(e) { setTabError('tab-olsr2'); }
          }).catch(function(){ setTabError('tab-olsr2'); });
        } catch(e) { setTabError('tab-olsr2'); }
      }
