...
function populateOlsr2Tab(data) {
  ...
  // Ensure we don't leave previous tables around (prevents multiplication when this
  // function is invoked multiple times or when tabs are switched rapidly).
  try {
    var prev = document.querySelector('#tab-olsr2 #olsr2LinksTable');
  ...
  // Insert the assembled definition list into the olsr2 content container so it
  // becomes part of the DOM. Previously this step was missing which could leave
  // orphaned nodes or cause duplicate insertions elsewhere when the tab was
  // re-rendered.
