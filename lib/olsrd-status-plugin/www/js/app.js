...
if (!window.Promise) {
  window.Promise = function(executor) {
    ...
    function resolve(value) {
      ...
      self._value = value;
      self._onFulfilled.forEach(function(fn){ try{ fn(value); }catch(e){} });
    }
    function reject(reason) {
      if (self._status !== 'pending') return;
      self._status = 'rejected';
      self._reason = reason;
      self._onRejected.forEach(function(fn){ try{ fn(reason); }catch(e){} });
    }
    try {
  ...
  window.Promise.prototype.then = function(onFulfilled, onRejected) {
    ...
    return new window.Promise(function(resolve, reject) {
      ...
      if (self._status === 'fulfilled') handleFulfilled(self._value);
      else if (self._status === 'rejected') handleRejected(self._reason);
      else { self._onFulfilled.push(handleFulfilled); self._onRejected.push(handleRejected); }
    });
  };
}
...
// Footer diagnostics drawer: open/close and populate diagnostics
(function(){
  ...
  // DEPRECATION NOTE: makeBox is a small diagnostics helper earmarked for
  // removal in a future cleanup. Keep it for now, but emit a one-time runtime
  // warning if used so we can detect live usages before removing it.
  function makeBox(title){
    try { if (!window._deprecated_warned_makeBox) { console.warn('DEPRECATION: makeBox() is flagged for removal in future refactors'); window._deprecated_warned_makeBox = true; } } catch(e){}
  ...
  // DEPRECATION NOTE: makeCollapsibleBox wraps makeBox and is flagged for
  // removal; emitting a one-time runtime warning when used.
  function makeCollapsibleBox(title){
    try { if (!window._deprecated_warned_makeCollapsibleBox) { console.warn('DEPRECATION: makeCollapsibleBox() is flagged for removal in future refactors'); window._deprecated_warned_makeCollapsibleBox = true; } } catch(e){}
    var wrap = document.createElement('div'); wrap.className = 'diag-box';
    var header = document.createElement('div'); header.className = 'diag-box-header';
    var t = document.createElement('div'); t.className='diag-title'; t.textContent = title; header.appendChild(t);
    var btns = document.createElement('div');
    var copyBtn = document.createElement('button'); copyBtn.className='btn btn-xs btn-default diag-copy-btn'; copyBtn.textContent='Copy';
  ...
  // DEPRECATION NOTE: kvRow is a small helper for diagnostics rendering. It
  // will be removed after confirming there are no live users. Emit a one-time
  // warning on first invocation to detect usage.
  function kvRow(k,v){
    try { if (!window._deprecated_warned_kvRow) { console.warn('DEPRECATION: kvRow() is flagged for removal in future refactors'); window._deprecated_warned_kvRow = true; } } catch(e){}
  ...
  function renderDiagnostics(payloads){
    ...
    // also populate status tab versions string (if present)
    try {
      var sEl = document.getElementById('status-versions-string');
      if (sEl) {
        var s = null;
        try { if (payloads && payloads.versions && payloads.versions.versions_string) s = payloads.versions.versions_string; } catch(e){}
        if (!s) try { if (payloads && payloads.versions_string) s = payloads.versions_string; } catch(e){}
        if (s) { sEl.textContent = String(s); } else { sEl.textContent = '\u00A0'; }
      }
    } catch(e) {}
      // set prominent versions/timestamp if available
    try {
      var vsEl = document.getElementById('diag-versions-string');
      if (vsEl) {
        var vs = null;
        try { if (payloads && payloads.versions && payloads.versions.versions_string) vs = payloads.versions.versions_string; } catch(e){}
        if (!vs) try { if (payloads && payloads.versions_string) vs = payloads.versions_string; } catch(e){}
          if (vs) { vsEl.textContent = String(vs); }
  ...
  (function(k, v, lineStr){ card.addEventListener('click', function(ev){ try { var pre = document.getElementById('diag-expand-pre'); if (pre) pre.textContent = JSON.stringify({key:k, value:v, raw:lineStr}, null, 2); var modal = document.getElementById('diag-expand-modal'); if (modal) { modal.setAttribute('aria-hidden','false'); } } catch(e){} ev.stopPropagation(); }); })(key, val, line);
      card.appendChild(kdiv); card.appendChild(vdiv);
      grid.appendChild(card);
    }
    body.appendChild(grid);
    // small snapshot line
  var meta = document.createElement('div'); meta.className='diag-small diag-snapshot'; meta.textContent = 'Snapshot at: ' + new Date().toLocaleString(); body.appendChild(meta);
  ...
  // diagnostics endpoint completely unavailable: fetch fallback diagnostics endpoints
      var tasks = [
        fetch('/versions.json',{cache:'no-store'}).then(r => r.text()).then(t => { try { return JSON.parse(t); } catch(e) { return {error: 'JSON parse error: ' + e.message}; } }).then(j=>p.versions=j).catch(function(e){ p.versions = {error: String(e)}; }),
        fetch('/capabilities',{cache:'no-store'}).then(r => r.text()).then(t => { try { return JSON.parse(t); } catch(e) { return {error: 'JSON parse error: ' + e.message}; } }).then(j=>p.capabilities=j).catch(function(e){ p.capabilities={error:String(e)}; }),
        fetch('/fetch_debug',{cache:'no-store'}).then(r => r.text()).then(t => { try { return JSON.parse(t); } catch(e) { return {error: 'JSON parse error: ' + e.message}; } }).then(j=>p.fetch_debug=j).catch(function(e){ p.fetch_debug={error:String(e)}; }),
  ...
  footer.classList.add('footer-clickable');
    ...
    // modal close
  var modalClose = el('diag-expand-close'); if (modalClose) modalClose.addEventListener('click', function(ev){ hideModal('diag-expand-modal'); ev.stopPropagation(); });
  // hide modal on overlay click
  var modalOverlay = el('diag-expand-modal'); if (modalOverlay) modalOverlay.addEventListener('click', function(ev){ if (ev.target === modalOverlay) { hideModal('diag-expand-modal'); } });
  });
})();
...
// --- WebUI request logging wrapper and Diagnostics modal wiring ---
// Wrap fetch to record webui request/response for diagnostics (bounded log)
(function(){
  if (!window.fetch) return;
  var _origFetch = window.fetch;
  window._webui_request_log = window._webui_request_log || [];
  window._webui_request_log_max = 50; // keep last 50 entries
  window.fetch = function(input, init){
    var t0 = Date.now();
    var method = (init && init.method) ? init.method : 'GET';
    var url = (typeof input === 'string')? input : (input && input.url) ? input.url : String(input);
    var logEntry = { id: Date.now()+Math.random(), url: url, method: method, request_init: { headers: init && init.headers? init.headers : undefined }, start: t0 };
    return _origFetch.apply(this, arguments).then(function(resp){
      var t1 = Date.now();
      try {
        var clone = resp.clone();
        clone.text().then(function(bodyText){
          logEntry.duration_ms = t1 - t0;
          logEntry.status = resp.status;
          logEntry.response_sample = bodyText && bodyText.length>10240 ? bodyText.substring(0,10240)+'...': bodyText;
          window._webui_request_log.push(logEntry);
          while(window._webui_request_log.length>window._webui_request_log_max) window._webui_request_log.shift();
        }).catch(function() {
          logEntry.duration_ms = t1 - t0; logEntry.status = resp.status; window._webui_request_log.push(logEntry);
          while(window._webui_request_log.length>window._webui_request_log_max) window._webui_request_log.shift();
        });
      } catch(e) { try{ logEntry.duration_ms = t1 - t0; logEntry.status = resp.status; window._webui_request_log.push(logEntry); }catch(_){} }
      return resp;
    }).catch(function(err){ var t1 = Date.now(); try{ logEntry.duration_ms = t1 - t0; logEntry.error = String(err); window._webui_request_log.push(logEntry); while(window._webui_request_log.length>window._webui_request_log_max) window._webui_request_log.shift(); }catch(_){}; throw err; });
  };
})();

// Diagnostics modal wiring (opened from footer-diagnostics button)
  document.addEventListener('DOMContentLoaded', function(){
    try {
      // support both legacy id 'footer-diagnostics' and the current 'footer-diagnostics-handle'
      var diagBtn = document.getElementById('footer-diagnostics') || document.getElementById('footer-diagnostics-handle');
      ...
      // open modal when handle/button clicked
      if (diagBtn && modal && content) {
        diagBtn.addEventListener('click', function(ev){ ev.stopPropagation(); modal.classList.remove('d-none'); renderDiagPage('intervals'); });
      }
      if (closeBtn && modal) {
        closeBtn.addEventListener('click', function(ev){ ev.stopPropagation(); modal.classList.add('d-none'); });
      }
      // wire top-nav buttons inside modal
      document.querySelectorAll('.diag-nav').forEach(function(b){ b.addEventListener('click', function(ev){ ev.stopPropagation(); var p = b.getAttribute('data-page'); renderDiagPage(p); }); });

    function renderDiagPage(page){
      try {
        content.innerHTML = '';
        if (page === 'params') {
          var h = document.createElement('div'); h.textContent = 'Parameters (runtime PlParams / env derived)'; content.appendChild(h);
          var info = document.createElement('div'); info.textContent = 'Loading server-side parameters...'; content.appendChild(info);
          // Fetch combined diagnostics payload and render selected parameter groups
          fetch('/diagnostics.json', {cache:'no-store'}).then(function(res){ return res.text(); }).then(function(txt){
            try {
              var j = JSON.parse(txt);
            } catch(e) {
              try { j = JSON.parse((txt||'').trim().split(/\n+/).pop()); } catch(_) { j = null; }
            }
            content.innerHTML = '';
            content.appendChild(h);
            if (!j) {
              var pre = document.createElement('pre'); pre.className = 'diag-compact-pre diag-raw-pre'; pre.textContent = 'Failed to parse /diagnostics.json'; content.appendChild(pre); return;
            }
            function addGroup(title, obj, groupKey) {
              // helper to format interval-like values in seconds
              function formatParamValue(k, rawVal, effective) {
                try {
                  if (rawVal === null || typeof rawVal === 'undefined' || rawVal === '') {
                    // apply sensible defaults for common intervals when value missing
                    if (/fetch_auto_refresh/i.test(k)) return '60 s (default)';
                    if (/links?.*interval/i.test(k)) return '10 s (default)';
                    if (/_ms$/i.test(k)) return '0 s';
                    return String(rawVal);
                  }
                  var num = Number(rawVal);
                  if (!isNaN(num)) {
                    if (/_ms$/i.test(k) || /_ms_/i.test(k) || /_ms/i.test(k) && k.toLowerCase().indexOf('ms')>-1) {
                      // milliseconds -> seconds
                      var s = num / 1000;
                      if (s === Math.floor(s)) return String(Math.floor(s)) + ' s';
                      return String(s.toFixed(2)) + ' s';
                    }
                    if (/_s$/i.test(k) || /interval/i.test(k) || /ttl/i.test(k)) {
                      return String(num) + ' s';
                    }
                  }
                  return String(rawVal);
              ...
              var box = document.createElement('div'); box.className = 'diag-box';
              var t = document.createElement('div'); t.className = 'diag-subtitle'; t.textContent = title; box.appendChild(t);
              var table = document.createElement('table'); table.className='table table-condensed diag-param-table';
              var tbody = document.createElement('tbody');
              // params_meta may provide friendly labels and descriptions
              var meta = (j && j.params_meta && groupKey && j.params_meta[groupKey]) ? j.params_meta[groupKey] : null;
              Object.keys(obj).sort().forEach(function(k){
                // row contains label, value, and actions
                var tr = document.createElement('tr');
                  var td1 = document.createElement('td'); td1.className = 'param-label-cell';
                  var displayKey = k;
                var descText = '';
                var plparam = null; var envvar = null; var effective = null; var desc_key = null;
                if (meta && meta[k]) {
                  try {
                    var m = meta[k];
                    if (m.label) displayKey = m.label;
                    if (m.desc) descText = m.desc;
                    if (m.plparam) plparam = m.plparam;
                    if (m.env) envvar = m.env;
                    if (m.effective) effective = m.effective; // server may supply computed source
                    if (m.desc_key) desc_key = m.desc_key;
                  } catch(e){}
                }
                // left column: display label + small meta line
                td1.textContent = displayKey;
                var metaLine = document.createElement('div'); metaLine.className = 'param-meta-line';
                var parts = [];
                if (plparam) parts.push('PlParam: ' + plparam);
                if (envvar) parts.push('Env: ' + envvar);
                if (parts.length) metaLine.textContent = parts.join(' • ');
                td1.appendChild(metaLine);

                var td2 = document.createElement('td'); td2.className = 'param-value-cell';
                var valSpan = document.createElement('div');
                try {
                  if (typeof obj[k] === 'object') valSpan.textContent = JSON.stringify(obj[k], null, 2);
                  else valSpan.textContent = formatParamValue(k, obj[k], effective);
                } catch(e){ valSpan.textContent = formatParamValue(k, obj[k], effective); }
                td2.appendChild(valSpan);

                // action column: copy button and effective source badge
                var actions = document.createElement('div'); actions.className = 'param-actions';
                var copyBtn = document.createElement('button'); copyBtn.className='btn btn-xs btn-default'; copyBtn.textContent='Copy'; copyBtn.title='Copy value to clipboard';
                copyBtn.addEventListener('click', function(){
                  try {
                    var text = valSpan.textContent || '';
                  ...
                  } catch(e) { try{ copyBtn.textContent = 'Err'; setTimeout(function(){ copyBtn.textContent = 'Copy'; }, 1200); }catch(_){} }
                });
                actions.appendChild(copyBtn);
                if (!effective) { if (meta && meta[k] && meta[k].effective_source) effective = meta[k].effective_source; }
                if (effective) {
                  var badge = document.createElement('span'); badge.className = 'param-effective-badge'; badge.textContent = String(effective);
                  actions.appendChild(badge);
                }
                td2.appendChild(actions);
                tr.appendChild(td1); tr.appendChild(td2); tbody.appendChild(tr);
                // description (i18n aware)
                var localDesc = descText || '';
                if (desc_key && window._i18n && window._i18n[desc_key]) localDesc = window._i18n[desc_key];
                if (localDesc) {
                  var trd = document.createElement('tr'); var dtd = document.createElement('td'); dtd.colSpan=2; dtd.className = 'param-desc-cell'; dtd.textContent = localDesc; trd.appendChild(dtd); tbody.appendChild(trd);
                }
              });
              table.appendChild(tbody); box.appendChild(table); content.appendChild(box);
            }

            // Preferred groups
            if (j.globals) addGroup('Globals', j.globals, 'globals');
            if (j.fetch_opts) addGroup('Fetch options', j.fetch_opts, 'fetch_opts');
            if (j.fetch) addGroup('Fetch queue settings', j.fetch, 'fetch');
            if (j.ubnt) addGroup('UBNT', j.ubnt, 'ubnt');
            if (j.arp) addGroup('ARP cache', j.arp, 'arp');
            if (j.coalesce) addGroup('Coalescing', j.coalesce, 'coalesce');
            if (j.debug) addGroup('Debug flags', j.debug, 'debug');

            // Show an "All parameters" section listing entries from params_meta so users can see every possible parameter
            if (j.params_meta) {
              var allBox = document.createElement('div'); allBox.className = 'all-params-box';
              var ah = document.createElement('div'); ah.className = 'param-all-heading'; ah.textContent = 'All parameters (from params_meta)'; allBox.appendChild(ah);
              var at = document.createElement('table'); at.className='table table-condensed mt-6'; var atb = document.createElement('tbody');
              var pmKeys = Object.keys(j.params_meta).sort();
              // recursive search for value by key across payload
              function findValueRecursive(obj, key) {
                ...
                for (var p in obj) {
                  ...
                  try {
                    ...
                    if (v && typeof v === 'object') {
                      var found = findValueRecursive(v, key);
              ...
              pmKeys.forEach(function(k){
                var m = j.params_meta[k] || {};
                var displayKey = m.label || k;
                var rawVal = findValueRecursive(j, k);
                var tr = document.createElement('tr');
                var td1 = document.createElement('td'); td1.className = 'param-label-cell'; td1.textContent = displayKey;
                var td2 = document.createElement('td');
                var vdiv = document.createElement('div'); vdiv.textContent = (typeof rawVal === 'undefined') ? '(not set)' : formatParamValue(k, rawVal, m.effective || m.effective_source);
                td2.appendChild(vdiv);
                var metaLine = document.createElement('div'); metaLine.className = 'param-meta-line';
                var parts = [];
                if (m.plparam) parts.push('PlParam: ' + m.plparam);
                if (m.env) parts.push('Env: ' + m.env);
                if (parts.length) metaLine.textContent = parts.join(' • ');
                td1.appendChild(metaLine);
                tr.appendChild(td1); tr.appendChild(td2); atb.appendChild(tr);
              });
              at.appendChild(atb); allBox.appendChild(at); content.appendChild(allBox);
            }

            // fallback: dump the full diagnostics object into a collapsible panel
            var rawBox = document.createElement('div'); rawBox.className = 'mt-8'; var rawBtn = document.createElement('button'); rawBtn.className='btn btn-xs btn-default'; rawBtn.textContent='Show full diagnostics JSON'; rawBox.appendChild(rawBtn);
            var rawPre = document.createElement('pre'); rawPre.className = 'pre-wrap d-none'; rawPre.textContent = JSON.stringify(j, null, 2);
            rawBtn.addEventListener('click', function(){ rawPre.classList.toggle('d-none'); });
            content.appendChild(rawBox); content.appendChild(rawPre);
          }).catch(function(e){
            // network or parse error: show local parameters
            content.innerHTML = '';
            content.appendChild(h);
            var t = document.createElement('pre'); t.className = 'pre-wrap'; t.textContent = JSON.stringify({ fetch_auto_refresh_ms: window._fetch_auto_refresh_ms || null, links_poll_interval_ms: window._links_poll_interval_ms || null }, null, 2);
            content.appendChild(t);
          });
        } else if (page === 'intervals') {
          var row = document.createElement('div'); row.className = 'flex-col-gap-8';
          var lbl = document.createElement('div'); lbl.textContent='Polling intervals — change and they take effect immediately'; row.appendChild(lbl);
          function addRow(labelText, localKey, defaultVal, opts) {
            opts = opts || {};
            var wr = document.createElement('div'); wr.className = 'flex-row-gap-8';
            var kLbl = document.createElement('div'); kLbl.className = 'w-220'; kLbl.textContent = labelText; wr.appendChild(kLbl);
            var inp = document.createElement('input'); inp.type='number'; inp.min='1'; inp.className = 'w-120';
            var stored = localStorage.getItem(localKey);
            if (stored !== null) inp.value = stored; else inp.value = String(defaultVal);
            inp.addEventListener('change', function(){
              ...
              try {
                if (localKey === 'links_live_interval_s') {
                  window._links_poll_interval_ms = v * 1000;
                  if (typeof stopLinksPoll === 'function' && typeof startLinksPoll === 'function') { try{ stopLinksPoll(); startLinksPoll(); } catch(e){} }
                } else if (localKey === 'fetch_auto_refresh_ms') {
                  // this value is expected in milliseconds
                  window._fetch_auto_refresh_ms = v;
                }
              } catch(e){}
            });
            wr.appendChild(inp);
            // helper note
            var note = document.createElement('div'); note.className = 'note-muted'; note.textContent = opts.note || '';
            wr.appendChild(note);
            row.appendChild(wr);
          }
          var linksDefault = Math.max(1, Math.floor((window._links_poll_interval_ms || 10000) / 1000));
          addRow('Links live interval (s)', 'links_live_interval_s', linksDefault, { note: 'seconds' });
          addRow('Status auto-refresh (ms)', 'fetch_auto_refresh_ms', window._fetch_auto_refresh_ms || 10000, { note: 'milliseconds' });
          content.appendChild(row);
        } else if (page === 'logging') {
          var tbl = document.createElement('div'); tbl.className = 'flex-col-gap-6';
          var logs = window._webui_request_log || [];
          if (logs.length === 0) { content.appendChild(document.createTextNode('No web-request logs yet.')); return; }
          // add a small filter box
          var filter = document.createElement('input'); filter.type='text'; filter.placeholder='Filter (method, url, status)'; filter.className='form-control input-sm mb-6';
          content.appendChild(filter);
          var listWrap = document.createElement('div'); listWrap.className = 'list-wrap';
          function renderList(q){
            listWrap.innerHTML = '';
            var items = logs.slice().reverse();
            if (q) items = items.filter(function(l){ return JSON.stringify(l).toLowerCase().indexOf(q.toLowerCase()) !== -1; });
            items.forEach(function(l){
              var r = document.createElement('div'); r.className = 'list-item';
              r.textContent = '['+ (l.status || l.error || '-') +'] ' + l.method + ' ' + l.url + ' ('+(l.duration_ms||'-')+'ms)';
              var details = document.createElement('pre'); details.className = 'list-item-details d-none';
              details.textContent = JSON.stringify(l, null, 2);
          ...
          renderList('');
          content.appendChild(tbl); content.appendChild(listWrap);
        }
      } catch(e) { try{ content.textContent = 'Failed to render page: '+e; }catch(_){} }
    }
  } catch(e) {}
});
...
function showModal(id) {
  ...
  try { m.classList.remove('d-none'); } catch(e) {}
}
function hideModal(id) {
  var m = document.getElementById(id);
  if (!m) return;
  try { m.setAttribute('aria-hidden','true'); } catch(e) {}
  try { m.classList.add('d-none'); } catch(e) {}
}
...
function showTab(tabId, show) {
  ...
  try {
  if (window._uiDebug) console.debug('showTab called for', tabId, 'show=', !!show, 'el.className=', el.className);
  } catch(e){}
}
...
function populateDevicesTable(devices, airos) {
  ...
  try { var pane = document.getElementById('tab-status'); if (pane) { pane.classList.remove('hidden'); pane.classList.remove('d-none'); } } catch(e) {}
  ...
  // Merge duplicates and aggregate by hostname so devices with the same
  // hostname appear as a single row and list all known IPs in the Local (IP / HW)
  // column. Fallback key order: hostname, hwaddr, ipv4/ip, anon.
  try {
    var byHost = Object.create(null);
    devices.forEach(function(dev) {
      var hostKey = (dev.hostname || '').toString().trim();
      var fallbackKey = null;
      if (!hostKey) {
        if (dev.hwaddr) fallbackKey = 'hw:' + String(dev.hwaddr).toLowerCase();
        else if (dev.ipv4) fallbackKey = 'ip:' + String(dev.ipv4);
        else if (dev.ip) fallbackKey = 'ip:' + String(dev.ip);
        else fallbackKey = 'anon:' + (Math.random().toString(36).slice(2,8));
      }
      var key = hostKey || fallbackKey;

      if (!byHost[key]) {
        var copy = {};
        for (var k in dev) if (dev.hasOwnProperty(k)) copy[k] = dev[k];
        copy._ips = new Set();
        var ipVal = dev.ipv4 || dev.ip || '';
        if (ipVal) copy._ips.add(String(ipVal));
        copy._hw = new Set(); if (dev.hwaddr) copy._hw.add(String(dev.hwaddr));
        copy._sources = new Set(); copy._sources.add(detectSource(dev));
        byHost[key] = copy;
      } else {
        var cur = byHost[key];
        // collect IPs and hwaddrs
        var ipVal = dev.ipv4 || dev.ip || '';
        if (ipVal) cur._ips.add(String(ipVal));
        if (dev.hwaddr) cur._hw.add(String(dev.hwaddr));
        cur._sources.add(detectSource(dev));
        // prefer non-empty values for other display fields
        ['hostname','product','uptime','mode','essid','firmware','signal','tx_rate','rx_rate'].forEach(function(f){
          try { if ((!cur[f] || String(cur[f]).trim() === '') && dev[f] && String(dev[f]).trim() !== '') cur[f] = dev[f]; } catch(e) {}
        });
      }
    });

    // rebuild devices array: for each host aggregate IPs into a comma-separated string
    var mergedDevices = [];
    Object.keys(byHost).forEach(function(k){
      var item = byHost[k];
      try {
        var srcs = Array.from(item._sources).sort().filter(function(x){return x && x!=='unknown';}).join(', ');
        item.source = srcs || 'unknown';
      } catch(e){ item.source = 'unknown'; }
      // build combined ipv4 string containing all distinct IPs (comma+space)
      try { item.ipv4 = Array.from(item._ips).join(', '); } catch(e) { item.ipv4 = item.ipv4 || ''; }
      // preserve primary hwaddr if available, otherwise show first
      try { item.hwaddr = Array.from(item._hw).join(', '); } catch(e) { item.hwaddr = item.hwaddr || ''; }
      delete item._ips; delete item._hw; delete item._sources;
  ...
  var localWrap = document.createElement('div'); localWrap.classList.add('fs-60');
  ...
  localWrap.appendChild(ipSpan); localWrap.appendChild(hwSpan); localIpTd.appendChild(localWrap);
    ...
    // attempt to populate missing signal/tx/rx fields from airos data when available
    try {
      if (airos && device && (!device.signal || String(device.signal).trim() === '' || !device.tx_rate || String(device.tx_rate).trim() === '' || !device.rx_rate || String(device.rx_rate).trim() === '')) {
        var a = airos[device.ipv4];
        if (a && Array.isArray(a.connections) && a.connections.length > 0) {
          // try to find a matching station by MAC or hostname
          var match = null;
          var devHw = (device.hwaddr || '').toLowerCase();
          var devName = (device.hostname || '').toString();
          for (var si = 0; si < a.connections.length; si++) {
            var s = a.connections[si];
            try {
              if (s.mac && devHw && s.mac.toLowerCase() === devHw) { match = s; break; }
              if (s.name && devName && s.name === devName) { match = s; break; }
            } catch(e){}
          }
          if (!match && a.connections.length === 1) match = a.connections[0];
          if (match) {
            try { if ((!device.signal || String(device.signal).trim()==='') && typeof match.signal !== 'undefined') device.signal = String(match.signal); } catch(e){}
            try { if ((!device.tx_rate || String(device.tx_rate).trim()==='') && typeof match.tx !== 'undefined') device.tx_rate = String(match.tx); } catch(e){}
            try { if ((!device.rx_rate || String(device.rx_rate).trim()==='') && typeof match.rx !== 'undefined') device.rx_rate = String(match.rx); } catch(e){}
          }
        }
      }
    } catch(e) {}
...
function populateOlsrLinksTable(links) {
  ...
  links.forEach(function(l){
    var tr = document.createElement('tr');
  if (l.is_default) { tr.classList.add('row-default-highlight'); }
  // use global td(val, allowHtml) helper
    tr.appendChild(td(l.intf));
    tr.appendChild(td(l.local));
    tr.appendChild(td(l.remote));
    // show remote_host if available, otherwise show remote IP
    try {
      // Resolve node name early (function is hoisted within this scope)
      var rnEarly = null;
      try { rnEarly = typeof resolveNodeNameForLink === 'function' ? resolveNodeNameForLink(l) : null; } catch(e) { rnEarly = null; }
      var primaryEarly = (rnEarly && rnEarly.name) ? rnEarly.name : '';
      if (l.remote_host && String(l.remote_host).trim()) {
        // If remote_host is short (no dot) and we have a node name, compose
        // hostname.node.domain; otherwise prefer remote_host as-is.
        var displayHost = String(l.remote_host).trim();
        if (displayHost.indexOf('.') === -1 && primaryEarly) {
          // If primaryEarly already contains a domain, append it; otherwise
          // append with the known site domain (wien.funkfeuer.at).
          if (primaryEarly.indexOf('.') === -1) {
            displayHost = displayHost + '.' + primaryEarly + '.wien.funkfeuer.at';
          } else {
            displayHost = displayHost + '.' + primaryEarly;
          }
        }
        var linkHtml = '<a target="_blank" href="https://' + displayHost + '">' + displayHost + '</a>';
        try {
        ...
        } catch(e) { tr.appendChild(td(displayHost)); }
      } else {
        tr.appendChild(td(l.remote));
      }
    } catch(e){ tr.appendChild(td(l.remote_host || l.remote)); }
  ...
  var costHtml = '<span class="metric-badge small badge-default">'+(costVal||'')+'</span>';
    if (!isNaN(costNum)) {
  costHtml = '<span class="metric-badge small badge-primary">'+costNum+'</span>';
    }
    try {
      var costTd = document.createElement('td');
  var costSpan = document.createElement('span'); costSpan.className = 'metric-badge small';
  if (!isNaN(costNum)) costSpan.classList.add('badge-primary'); else costSpan.classList.add('badge-default');
  ...
  routesCell.classList.add('clickable'); routesCell.title='Click to view routes via this neighbor';
  ...
  if (parseInt(nodeCount,10) > 0) { nodesCell.classList.add('clickable'); nodesCell.title = l.node_names; nodesCell.addEventListener('click', function(){ showNodesFor(l.remote, l.node_names); }); }
    }
  ...
  try { var pane = document.getElementById('tab-olsr'); if (pane && pane.classList.contains('active')) { /* no-op but forces style recalculation in some browsers */ pane.classList.remove('d-none'); setTimeout(function(){ pane.classList.remove('d-none'); }, 10); } } catch(e) {}
  // Also proactively remove any hidden class from the OLSR pane so content is visible
  try { var pane2 = document.getElementById('tab-olsr'); if (pane2) { pane2.classList.remove('hidden'); pane2.classList.remove('d-none'); } } catch(e) {}
  ...
  // Update the links count in the status header
  try { var olcount = (links && Array.isArray(links)) ? links.length : 0; var el = document.getElementById('olsr-links-count'); if (el) el.textContent = olcount; } catch(e) {}
}

function populateOlsr2LinksTable(links) {
  // Remove any stray #olsr2LinksTable instances that are not inside the tab-olsr2
  try {
    var allTables = Array.prototype.slice.call(document.querySelectorAll('#olsr2LinksTable'));
    var tabO2 = document.getElementById('tab-olsr2');
    allTables.forEach(function(t){ if (!tabO2 || !tabO2.contains(t)) { try { t.parentNode.removeChild(t); } catch(e){} } });
  } catch(e) {}

  // Prefer the table inside the olsr2 tab to avoid accidentally writing into other locations
  var tbody = document.querySelector('#tab-olsr2 #olsr2LinksTable tbody') || document.querySelector('#olsr2LinksTable tbody');
  if (!tbody) return; clearChildren(tbody);
  if (!links || !Array.isArray(links) || links.length === 0) {
    var tbody = document.querySelector('#olsr2LinksTable tbody');
    if (tbody) {
  clearChildren(tbody);
      var tr = document.createElement('tr');
      var tdNo = document.createElement('td'); tdNo.colSpan = 12; tdNo.className = 'text-muted'; tdNo.textContent = 'No OLSR2 links found';
      tr.appendChild(tdNo); tbody.appendChild(tr);
    }
    return;
  }
  links.forEach(function(l){
    var tr = document.createElement('tr');
    // Assume similar fields, but adjust as needed
    tr.appendChild(td(l.interface || l.intf || ''));
    tr.appendChild(td(l.local || ''));
    tr.appendChild(td(l.remote || ''));
    tr.appendChild(td(l.remote_host || l.remote || ''));
    tr.appendChild(td(l.node || ''));
    tr.appendChild(td(l.lq || ''));
    tr.appendChild(td(l.nlq || ''));
    tr.appendChild(td(l.cost || ''));
    tr.appendChild(td(l.etx || ''));
...
function normalizeOlsr2Links(obj) {
  // Accept multiple shapes returned by telnet bridge / olsr2:
  // - { links: [ { link: [ ... ] }, ... ] }
  // - { links: [ ... ] }
  // - [ ... ]
  // - { link: [ ... ] }
  if (!obj) return [];
  var out = [];
  // If already an array, attempt to flatten wrapper entries (e.g. [{link:[..]}])
  if (Array.isArray(obj)) {
    try {
      obj.forEach(function(it) {
        ...
        if (Array.isArray(it)) {
          ...
          out = out.concat(it.link);
        } else if (Array.isArray(it.links)) {
          out = out.concat(it.links);
  ...
  // If top-level has 'links' array
  if (Array.isArray(obj.links)) {
    obj.links.forEach(function(it){
      if (!it) return;
      if (Array.isArray(it)) {
        out = out.concat(it);
      } else if (Array.isArray(it.link)) {
  ...
  // If top-level has 'link' array
  if (Array.isArray(obj.link)) return obj.link;
  // If top-level has 'links' object with 'link' array
  if (obj.links && Array.isArray(obj.links.link)) return obj.links.link;
  // Last attempt: if object has numeric keys treat as array-like
  try {
    for (var k in obj) {
...
function showNodesFor(remoteIp, nodeNames) {
  ...
  if (countBadge) { countBadge.classList.add('d-none'); countBadge.classList.remove('d-inline-block'); countBadge.textContent=''; }
  if (tbody) {
  clearChildren(tbody);
  ...
  if (bodyPre) { bodyPre.classList.add('d-none'); bodyPre.classList.remove('d-block'); bodyPre.textContent='Loading...'; }
  ...
  var tdIp = document.createElement('td'); tdIp.className = 'mono'; tdIp.textContent = n.ip || ''; tr.appendChild(tdIp);
  ...
  if (countBadge) { countBadge.classList.remove('d-none'); countBadge.classList.add('d-inline-block'); countBadge.textContent = found.length; }
    renderRows(found);
...
function showRoutesFor(remoteIp) {
  ...
  var tdDest = document.createElement('td'); tdDest.className = 'mono'; tdDest.textContent = r.destination || ''; tr.appendChild(tdDest);
  ...
  var tdRaw = document.createElement('td'); tdRaw.className = 'mono td-muted'; tdRaw.textContent = r.raw || ''; tr.appendChild(tdRaw);
  tr.tabIndex = 0; tr.classList.add('clickable');
      tbody.appendChild(tr);
    }
  }
  ...
  if (countBadge) { countBadge.classList.remove('d-none'); countBadge.classList.add('d-inline-block'); countBadge.textContent = (obj && typeof obj.count==='number'? obj.count : allRoutes.length); }
...
// IPv6 hostname resolution function
function resolveIPv6Hostname(ipv6) {
  // First try to get from nodedb cache
  if (window._nodedb_cache && window._nodedb_cache[ipv6]) {
    var node = window._nodedb_cache[ipv6];
    if (node.n) {
      return node.n;
    }
    if (node.d) {
  ...
  // Check if it's a known IPv6 format and try to resolve
  // For now, since we can't do DNS from browser, show the compressed IPv6
  try {
    // Simple IPv6 compression
    var compressed = ipv6.replace(/(:0)+$/, '').replace(/:0/g, ':').replace(/:{2,}/g, '::');
    return compressed;
  } catch(e) {
    return ipv6;
  }
...
function populateOlsr2Tab(data) {
  var content = document.getElementById('olsr2-content');
  if (!content) return;
  // Simple render lock to prevent concurrent re-renders when updates/fetches
  // overlap or when users rapidly switch tabs. If a render is in progress,
  // skip this invocation; another update will follow shortly from the next
  // data tick.
  try {
    window._olsr2_render_lock = window._olsr2_render_lock || false;
    if (window._olsr2_render_lock) return; // busy
    window._olsr2_render_lock = true;
  } catch(e) { /* best-effort */ }
  
  // Ensure we're appending to the correct tab
  var tabOlsr2 = document.getElementById('tab-olsr2');
  if (tabOlsr2 && !tabOlsr2.contains(content)) {
    // If olsr2-content is not in the correct tab, try to find it and move it back
    if (content.parentNode && content.parentNode !== tabOlsr2) {
      // Remove from wrong parent and append to correct location
      content.parentNode.removeChild(content);
    }
    // Find the h3 and append after it
    var h3 = tabOlsr2.querySelector('h3');
    if (h3) {
      h3.parentNode.insertBefore(content, h3.nextSibling);
    } else {
      tabOlsr2.appendChild(content);
    }
  }
  ...
  dt1.innerHTML = 'Olsrd2 Version <span class="glyphicon glyphicon-time" aria-hidden="true"></span>';
  var dd1 = document.createElement('dd');
  try {
    var version = data.olsr2_version && data.olsr2_version.version && data.olsr2_version.version[0];
    dd1.textContent = (version ? version.version_text || 'unknown' : 'unknown') + ' | ' + (version ? version.version_commit || 'unknown' : 'unknown');
  } catch(e) {
    dd1.textContent = 'unknown-version';
  }
  dl.appendChild(dt1);
  dl.appendChild(dd1);
  ...
  dt2.innerHTML = 'Olsrd2 Uptime <span class="glyphicon glyphicon-time" aria-hidden="true"></span>';
  var dd2 = document.createElement('dd');
  try {
    var timeData = data.olsr2_time && data.olsr2_time.time && data.olsr2_time.time[0];
    var uptime = timeData ? timeData.time_internal || 'unknown' : 'unknown';
    var uptimeStr = timeData ? timeData.time_system || 'unknown' : 'unknown';
    dd2.textContent = uptimeStr.split('.')[0] + ' | ' + uptime;
  } catch(e) {
    dd2.textContent = 'unknown-time';
  }
  dl.appendChild(dt2);
  dl.appendChild(dd2);
  ...
  dt3.innerHTML = 'OLSRd2 Originator <span class="glyphicon glyphicon-time" aria-hidden="true"></span>';
  var dd3 = document.createElement('dd');
  try {
    var originator = data.olsr2_originator && data.olsr2_originator.originator && data.olsr2_originator.originator[1];
    dd3.textContent = originator ? originator.originator || 'unknown' : 'unknown';
  } catch(e) {
    dd3.textContent = 'unknown-originator';
  }
  dl.appendChild(dt3);
  dl.appendChild(dd3);
  ...
  dt4.innerHTML = 'IPv6 Default-Route <span class="glyphicon glyphicon-transfer" aria-hidden="true"></span>';
  var dd4 = document.createElement('dd');
  dd4.textContent = (data.def6_ip || 'i?') + ' via ' + (data.def6_dev || 'd?');
  dl.appendChild(dt4);
  dl.appendChild(dd4);
  
  // IPv6 OLSR2 Links
  var dt5 = document.createElement('dt');
  dt5.innerHTML = 'IPv6 OLSR2-Links <span class="glyphicon glyphicon-link" aria-hidden="true"></span>';
  var dd5 = document.createElement('dd');
  
  // Add search input like OLSR Links table
  var searchContainer = document.createElement('div');
  searchContainer.className = 'table-search-container';
  var searchInput = document.createElement('input');
  searchInput.className = 'table-search-input form-control input-sm';
  searchInput.setAttribute('data-target', '#olsr2LinksTable');
  searchInput.placeholder = 'Search...';
  searchContainer.appendChild(searchInput);
  dd5.appendChild(searchContainer);
  
  // Add table responsive wrapper
  var tableResponsive = document.createElement('div');
  tableResponsive.className = 'table-responsive';
  
  // Create table for OLSR2 links with same styling as OLSR Links
  // Prefer re-using a single persistent table instance when possible. Store a
  // reference on window so we don't recreate table nodes (and their handlers)
  // on every render which can cause duplicates or leaked handlers.
  try {
    window._olsr2_table_ref = window._olsr2_table_ref || null;
  } catch(e) { window._olsr2_table_ref = null; }

  var table = window._olsr2_table_ref;
  if (!table) {
    table = document.createElement('table');
    table.id = 'olsr2LinksTable';
    table.className = 'table table-condensed table-striped';
    window._olsr2_table_ref = table;
  } else {
    // If the table exists elsewhere in the DOM, ensure we move it under the
    // tab so querySelector('#tab-olsr2 #olsr2LinksTable tbody') works later.
    try { if (table.parentNode && !document.getElementById('tab-olsr2').contains(table)) { table.parentNode.removeChild(table); } } catch(e) {}
  }
  // Rebuild/ensure thead only when creating new table (idempotent for reuse)
  var thead = document.createElement('thead');
  var headerRow = document.createElement('tr');
  headerRow.innerHTML = '<th data-key="intf">Intf</th><th data-key="remote">Remote IPv6</th><th data-key="remote_host">Remote Hostname</th><th data-key="link_status">Link-Status</th><th data-key="metric">Metric</th><th data-key="routes">Routes</th><th data-key="nodes">Nodes</th>';
  thead.appendChild(headerRow);
  if (!table.querySelector('thead')) table.appendChild(thead);

  // Prepare tbody: reuse existing tbody if present, otherwise create and
  // attach a fresh one. Always clear its children before populating.
  ...
  try {
    var neighbors = data.olsr2_neighbors && data.olsr2_neighbors.link ? data.olsr2_neighbors.link : [];
    neighbors.forEach(function(link) {
      if (!link || !link.link_bindto) return;
      var tr = document.createElement('tr');
      if (link.link_bindto === data.def6_ip) {
        tr.classList.add('row-gold-highlight');
      }
      
      // Interface
      var td1 = document.createElement('td');
      td1.textContent = link['if'] || '';
      tr.appendChild(td1);
      
      // Remote IPv6
      var td2 = document.createElement('td');
      var remoteIp = link.neighbor_originator || '';
      var a = document.createElement('a');
      a.href = 'https://[' + remoteIp + ']';
      a.target = '_blank';
      a.textContent = remoteIp;
      td2.appendChild(a);
      tr.appendChild(td2);
      
      // Remote Hostname - resolve IPv6 address
      var td3 = document.createElement('td');
      if (remoteIp && remoteIp !== '') {
        // Try to resolve IPv6 hostname
        resolveIPv6Hostname(remoteIp, function(hostname) {
          td3.textContent = hostname || 'unknown';
        });
        td3.textContent = 'resolving...'; // Show loading state
      } else {
      ...
      td4.textContent = link.link_status || '';
      tr.appendChild(td4);
      
      // Metric
      var td5 = document.createElement('td');
      td5.innerHTML = 'In: ' + (link.domain_metric_in || '-') + '<br>Out: ' + (link.domain_metric_out || '-');
      tr.appendChild(td5);
      
      // Routes and Nodes - calculate based on available data
      var td6 = document.createElement('td');
      // Simple calculation: if it's a valid link, count as 1 route
      // In a full implementation, this would count actual routes through this neighbor
      td6.textContent = link.link_status === 'asymmetric' || link.link_status === 'symmetric' ? '1' : '0';
      tr.appendChild(td6);
      
      var td7 = document.createElement('td');
      // Simple calculation: count based on domain metrics
      // In a full implementation, this would count nodes reachable through this neighbor
      var nodesCount = 0;
      if (link.domain_metric_in && parseInt(link.domain_metric_in) > 0) nodesCount++;
      if (link.domain_metric_out && parseInt(link.domain_metric_out) > 0) nodesCount++;
      td7.textContent = nodesCount.toString();
      tr.appendChild(td7);
    ...
    var td = document.createElement('td');
    td.colSpan = 7;
    td.textContent = 'Error loading OLSR2 neighbors: ' + e.message;
    tr.appendChild(td);
    tbody.appendChild(tr);
  }
  ...
  dd5.appendChild(tableResponsive);
  dl.appendChild(dt5);
  dl.appendChild(dd5);
  // Ensure the dl is inserted into the content area
  try { content.appendChild(dl); } catch(e) { /* best-effort */ }

  // Also ensure the persistent table is placed inside the dd5 wrapper so it is
  // visible in the tab and not duplicated elsewhere.
  try {
    if (!dd5.contains(table)) dd5.appendChild(tableResponsive);
  } catch(e) {}

  // Release render lock
  try { window._olsr2_render_lock = false; } catch(e) {}
...
function updateUI(data) {
  ...
  try {
    if (data.default_route && (data.default_route.ip || data.default_route.dev || data.default_route.hostname)) {
      ...
      // If host is short (no dot), try to enrich using node hints or nodedb cache
      var displayHost = host;
      try {
        if (displayHost && displayHost.indexOf('.') === -1) {
          var nodeHint = data.node || data.nodename || data.node_name || data.net || '';
          if (!nodeHint && data.versions && data.versions.host) {
            try { nodeHint = String(data.versions.host).split('.')[0] || nodeHint; } catch(_) {}
          }
          var ipKey = ip || data.ip || data.ipv4 || '';
          var nodePart = nodeHint || '';
          try {
            if ((!nodePart || nodePart === 'nodename') && ipKey && window._nodedb_cache && window._nodedb_cache[ipKey] && window._nodedb_cache[ipKey].n) {
              nodePart = window._nodedb_cache[ipKey].n;
          ...
          if (!nodePart || nodePart === 'nodename') nodePart = '';
          if (nodePart) {
            if (nodePart.indexOf('.') !== -1 || nodePart.indexOf('wien.funkfeuer') !== -1) {
              displayHost = displayHost + '.' + nodePart;
            } else {
              displayHost = displayHost + '.' + nodePart + '.wien.funkfeuer.at';
            }
          }
        }
      } catch(e) { /* defensive */ }
      ...
      var parts = [];
      if (displayHost) parts.push('<a target="_blank" href="https://' + displayHost + '">' + displayHost + '</a>');
  ...
  // Add OLSRd and OLSRv2 info to status tab in info-item style
  try {
    var statusTab = document.getElementById('tab-status');
    if (statusTab) {
      var container = statusTab.querySelector('.versions-container');
      if (!container) {
        // Create if not exists
        container = document.createElement('div');
        container.className = 'versions-container';
        // Insert after status-card
        var statusCard = statusTab.querySelector('#status-card');
        if (statusCard) {
          statusCard.parentNode.insertBefore(container, statusCard.nextSibling);
        } else {
          statusTab.appendChild(container);
        }
      }
      // Clear existing dynamic items (keep the static ones)
      var dynamicItems = container.querySelectorAll('.info-item.dynamic, .info-item-compact.dynamic');
      dynamicItems.forEach(function(item) { item.remove(); });

      // Helper: create a compact left/right info item (smaller footprint)
      function addCompactInfo(label, value, opts) {
        opts = opts || {};
        var wrap = document.createElement('div'); wrap.className = 'info-item-compact dynamic';
        var left = document.createElement('div'); left.className = 'info-label';
        var ic = document.createElement('span'); ic.className = opts.iconClass || 'glyphicon glyphicon-info-sign'; ic.setAttribute('aria-hidden','true');
        var lbl = document.createElement('div'); lbl.textContent = label;
        left.appendChild(ic); left.appendChild(lbl);
        var right = document.createElement('div'); right.className = 'info-value';
        if (typeof value === 'string') right.textContent = value; else right.textContent = String(value);
        wrap.appendChild(left); wrap.appendChild(right);
        container.appendChild(wrap);
        return wrap;
      }

      // Add OLSRd Version (compact)
      addCompactInfo('OLSRd Version', (data.versions && data.versions.versions_string) ? data.versions.versions_string : 'unknown', { iconClass: 'glyphicon glyphicon-cog' });
      // Add OLSRd Uptime (already have System Uptime, but add OLSRd specific if different)
      // For now, skip, since System Uptime is there
      // Add OLSRv2 items if available
      if (data.olsr2_on && data.olsr2_version) {
        // OLSR2 Version (compact)
        var version = data.olsr2_version && data.olsr2_version.version && data.olsr2_version.version[0];
        addCompactInfo('OLSRv2 Version', (version ? (version.version_text || 'unknown') + ' | ' + (version.version_commit || 'unknown') : 'unknown'), { iconClass: 'glyphicon glyphicon-cog' });
        // OLSR2 Uptime
        var timeData = data.olsr2_time && data.olsr2_time.time && data.olsr2_time.time[0];
        var uptime = timeData ? timeData.time_internal || 'unknown' : 'unknown';
        var uptimeStr = timeData ? timeData.time_system || 'unknown' : 'unknown';
        addCompactInfo('OLSRv2 Uptime', uptimeStr.split('.')[0] + ' | ' + uptime, { iconClass: 'glyphicon glyphicon-time' });
        // OLSR2 Originator
        var originator = data.olsr2_originator && data.olsr2_originator.originator && data.olsr2_originator.originator[1];
        addCompactInfo('Originator', (originator ? (originator.originator || 'unknown') : 'unknown'), { iconClass: 'glyphicon glyphicon-user' });
        // IPv6 Default Route
        // prefer structured default_route_v6, but fall back to def6_ip/def6_dev provided by status when present
        var ipv6Route = data.default_route_v6 || data.default_route_ipv6 || data.default_route || null;
        if ((!ipv6Route || !ipv6Route.ip) && data.def6_ip) {
          ipv6Route = { ip: data.def6_ip, dev: data.def6_dev || '' };
        }
        var ipv6Text = 'n/a';
        try {
          if (ipv6Route && ipv6Route.ip && ipv6Route.ip.indexOf(':') !== -1) {
            var parts = [];
            if (ipv6Route.hostname) parts.push(ipv6Route.hostname);
            else parts.push(ipv6Route.ip);
            if (ipv6Route.dev) parts.push('via ' + ipv6Route.dev);
            ipv6Text = parts.join(' ');
          }
        } catch(e) { ipv6Text = 'n/a'; }
        addCompactInfo('IPv6 Default Route', ipv6Text, { iconClass: 'glyphicon glyphicon-road' });
      }
    }
  } catch(e) {}
  // Remove the old dl if present
  try {
    var oldDl = statusTab.querySelector('dl.dl-horizontal');
  ...
  // populate status tab versions string if present in status payload
  try {
    var sEl = document.getElementById('status-versions-string');
    if (sEl) {
      var sv = null;
      try { if (data && data.versions && data.versions.versions_string) sv = data.versions.versions_string; } catch(e){}
      if (!sv) try { if (data && data.versions_string) sv = data.versions_string; } catch(e){}
      sEl.textContent = sv ? String(sv) : '\u00A0';
    }
  } catch(e) {}
  // legacy OLSR tab visibility (if backend set olsrd_on or provided links while not olsr2)
  try {
    // Show OLSR tab whenever links are present or olsrd is running.
    // Keep tab visible if it was previously loaded to avoid UI flicker on intermittent empty payloads.
    var linkTab = document.querySelector('#mainTabs a[href="#tab-olsr"]');
    if (linkTab) {
  var currentlyVisible = !linkTab.parentElement.classList.contains('d-none');
      var shouldShow = (data && (data.olsrd_on || (data.links && data.links.length))) || currentlyVisible;
      if (shouldShow) {
  ...
  linkTab.parentElement.classList.remove('d-none');
      } else {
  linkTab.parentElement.classList.add('d-none');
      }
    }
  } catch(e){}
...
function updateStatsCharts() {
  try {
    ...
    if (window._charts.network) {
      ...
      window._charts.network.data.datasets[2].data = window._stats_series.olsr2_nodes.slice(-STATS_SERIES_MAX);
      window._charts.network.data.datasets[3].data = window._stats_series.olsr2_routes.slice(-STATS_SERIES_MAX);
...
function updateTrendIndicators(routesArr, nodesArr, version) {
  try {
    ...
    // Update trend elements with version suffix
    var routesTrendEl = document.getElementById('routes' + (version || '') + '-trend'); 
    if (routesTrendEl) {
    ...
    var nodesTrendEl = document.getElementById('nodes' + (version || '') + '-trend'); 
    if (nodesTrendEl) {
...
function renderCard(opts) {
  ...
  var heading = document.createElement('div'); heading.className = 'panel-heading panel-heading-flex';

  var titleWrap = document.createElement('div');
  if (opts.title) { var title = document.createElement('div'); title.className = 'panel-title'; title.textContent = opts.title; titleWrap.appendChild(title); }
  if (opts.subtitle) { var sub = document.createElement('div'); sub.className = 'panel-subtitle'; sub.textContent = opts.subtitle; titleWrap.appendChild(sub); }
    heading.appendChild(titleWrap);

  var headerRight = document.createElement('div'); headerRight.className = 'panel-header-right'; heading.appendChild(headerRight);
    panel.appendChild(heading);

  var body = document.createElement('div'); body.className = 'panel-body';
  if (opts.bodyClass) body.className += ' ' + opts.bodyClass;
  else if (opts.bodyStyle) {
      // Move dynamic cssText into a dynamic stylesheet class instead of inline styles
      try {
        if (!window._dynStyleCounter) window._dynStyleCounter = 1;
        var dynIdx = window._dynStyleCounter++;
        var dynClass = 'dyn-style-' + dynIdx;
        var styleEl = document.querySelector('style[data-dynamic-style]');
        if (!styleEl) { styleEl = document.createElement('style'); styleEl.setAttribute('data-dynamic-style','1'); document.head.appendChild(styleEl); }
        // append a CSS rule scoped to the generated class
        styleEl.appendChild(document.createTextNode('.' + dynClass + ' {' + opts.bodyStyle + '}'));
        body.classList.add(dynClass);
  } catch(e) { /* dynamic injection failed; intentionally avoid applying inline cssText to keep styles centralized */ }
  }
  ...
  if (opts.smallHelp) { var help = document.createElement('div'); help.className = 'small-muted small-help'; help.textContent = opts.smallHelp; body.appendChild(help); }
...
updateUI = function(data) {
  try {
    ...
    try {
      ...
      // Prefer global counts from backend over per-link sums
    ...
    // sample olsr2 routes/nodes if available
    try {
      var olsr2_routes = 0; var olsr2_nodes = 0;
      if (data.olsr2_neighbors && data.olsr2_neighbors.link && Array.isArray(data.olsr2_neighbors.link)) {
        olsr2_nodes = data.olsr2_neighbors.link.length;
      }
      if (data.olsr2_routing && data.olsr2_routing.routing && Array.isArray(data.olsr2_routing.routing)) {
        olsr2_routes = data.olsr2_routing.routing.length;
      }
      pushStat('olsr2_routes', olsr2_routes);
      pushStat('olsr2_nodes', olsr2_nodes);
    } catch(e) {}
    ...
    try {
      // IPv4 (OLSR) values
      var routes4Arr = window._stats_series.olsr_routes || [];
      var nodes4Arr = window._stats_series.olsr_nodes || [];
      var routes4Cur = routes4Arr.length ? routes4Arr[routes4Arr.length-1] : 0;
      var nodes4Cur = nodes4Arr.length ? nodes4Arr[nodes4Arr.length-1] : 0;
      var routes4El = document.getElementById('routes4-cur'); if (routes4El) routes4El.textContent = String(routes4Cur);
      var nodes4El = document.getElementById('nodes4-cur'); if (nodes4El) nodes4El.textContent = String(nodes4Cur);
      
      // IPv6 (OLSR2) values
      var routes6Arr = window._stats_series.olsr2_routes || [];
      var nodes6Arr = window._stats_series.olsr2_nodes || [];
      var routes6Cur = routes6Arr.length ? routes6Arr[routes6Arr.length-1] : 0;
      var nodes6Cur = nodes6Arr.length ? nodes6Arr[nodes6Arr.length-1] : 0;
      var routes6El = document.getElementById('routes6-cur'); if (routes6El) routes6El.textContent = String(routes6Cur);
      var nodes6El = document.getElementById('nodes6-cur'); if (nodes6El) nodes6El.textContent = String(nodes6Cur);
      
      // Legacy combined values (for backward compatibility)
      var routesCur = routes4Cur + routes6Cur;
      var nodesCur = nodes4Cur + nodes6Cur;
      ...
      try {
        var memArr = window._stats_series.memory_used || [];
        var memCur = memArr.length ? memArr[memArr.length-1] : null;
        if (memCur === null && data && (typeof data.memory_used_percent !== 'undefined')) memCur = Number(data.memory_used_percent);
        if (memEl) memEl.textContent = (memCur === null || typeof memCur === 'undefined') ? 'N/A' : (Number(memCur).toFixed(1) + '%');
      } catch(e) { if (memEl) memEl.textContent = 'N/A'; }
      ...
      // Update trend indicators for IPv4 and IPv6
      updateTrendIndicators(routes4Arr, nodes4Arr, '4');
      updateTrendIndicators(routes6Arr, nodes6Arr, '6');
      // Legacy combined trend (for backward compatibility)
      updateTrendIndicators(routes4Arr.map((v,i) => v + (routes6Arr[i] || 0)), nodes4Arr.map((v,i) => v + (nodes6Arr[i] || 0)));
    } catch(e) {}
  } catch(e) {}
  ...
  function _statsPollOnce() {
    fetch('/status/lite', {cache:'no-store'}).then(function(r){ return r.json(); }).then(function(s){ try { if (s) {
      ...
      // If lite payload lacks node/route counts, fall back to /status/stats for those values
      if (typeof mapped.olsr_routes_count === 'undefined' || typeof mapped.olsr_nodes_count === 'undefined') {
        fetch('/status/stats', {cache:'no-store'}).then(function(r2){ return r2.json(); }).then(function(st){ try {
          if (st) {
            if (typeof st.olsr_routes_count !== 'undefined') mapped.olsr_routes_count = st.olsr_routes_count;
            if (typeof st.olsr_nodes_count !== 'undefined') mapped.olsr_nodes_count = st.olsr_nodes_count;
            if (st.fetch_stats && !mapped.fetch_stats) mapped.fetch_stats = st.fetch_stats;
            // Push memory stats from /status/stats when available
            if (typeof st.memory_used_percent !== 'undefined') pushStat('memory_used', Number(st.memory_used_percent));
            if (typeof st.memory_free_kb !== 'undefined' && typeof st.memory_total_kb !== 'undefined') {
              var freePerc = (Number(st.memory_free_kb) / Number(st.memory_total_kb)) * 100.0;
              pushStat('memory_free', freePerc);
            }
          }
          updateUI(mapped);
        } catch(e) { updateUI(mapped); } }).catch(function(){ updateUI(mapped); });
      } else {
        // Push memory stats from /status/lite when available
        if (typeof s.memory_used_percent !== 'undefined') pushStat('memory_used', Number(s.memory_used_percent));
        if (typeof s.memory_free_kb !== 'undefined' && typeof s.memory_total_kb !== 'undefined') {
          var freePerc2 = (Number(s.memory_free_kb) / Number(s.memory_total_kb)) * 100.0;
          pushStat('memory_free', freePerc2);
        }
        updateUI(mapped);
      }
  ...
  // DEPRECATION: _stopStatsPoll is kept for backward compatibility; flag uses.
  function _stopStatsPoll(){ try { if (!window._deprecated_warned__stopStatsPoll) { console.warn('DEPRECATION: _stopStatsPoll() flagged for removal'); window._deprecated_warned__stopStatsPoll = true; } } catch(e){} if (window._stats_poll_handle) { clearInterval(window._stats_poll_handle); window._stats_poll_handle = null; } }

// Links live polling: fetch '/status/links_live' periodically and update table
if (typeof window._links_poll_interval_ms === 'undefined') window._links_poll_interval_ms = 10000; // 10s default
function _linksPollOnce() {
  try {
    // Only poll when OLSR Links tab is visible to avoid unnecessary network
    var pane = document.getElementById('tab-olsr');
  if (!pane || pane.classList.contains('d-none') || pane.classList.contains('hidden')) return;
    fetch('/status/links_live', {cache:'no-store'}).then(function(r){ if (!r || !r.ok) throw new Error('HTTP '+(r&&r.status)); return r.json(); }).then(function(obj){
      try {
        if (obj && obj.links && Array.isArray(obj.links)) {
          window._olsr_links = obj.links.slice();
          try { populateOlsrLinksTable(window._olsr_links); } catch(e) {}
        }
      } catch(e) {}
    }).catch(function(){ /* ignore transient errors */ });
  } catch(e) {}
}
function startLinksPoll() {
  if (window._links_poll_handle) return;
  // pick up user-configured interval if present
  try {
    var iv = parseInt(localStorage.getItem('links_live_interval_s'), 10);
    if (!isNaN(iv) && iv > 0) window._links_poll_interval_ms = iv * 1000;
  } catch(e) {}
  try { _linksPollOnce(); } catch(e) {}
  window._links_poll_handle = setInterval(_linksPollOnce, window._links_poll_interval_ms);
}
function stopLinksPoll() { if (window._links_poll_handle) { clearInterval(window._links_poll_handle); window._links_poll_handle = null; } }
...
function showActionToast(msg, ms) {
  ...
  window._actionToastTimer = setTimeout(function(){ try { toast.classList.remove('toast-visible'); setTimeout(function(){ toast.classList.add('d-none'); },300); } catch(e){} }, ms || 2000);
  } catch(e) {}
}
...
function populateFetchStats(fs) {
  ...
  container.classList.add('fetch-stats-container');
      tabWrap.appendChild(container);
    }
  if (!fs || (typeof fs === 'object' && Object.keys(fs).length === 0)) { container.classList.add('d-none'); return; }
  ...
  var left = document.createElement('div'); left.className = 'flex-col-gap-8 flex-left-62';
  var badges = document.createElement('div'); badges.className = 'flex-gap-8';
  function mkBadgeNode(label, val, cls) { var sp=document.createElement('span'); sp.className='fetch-badge '+(cls||''); var lbl=document.createElement('span'); lbl.textContent = label + ':'; var strong=document.createElement('strong'); strong.textContent = String(val); sp.appendChild(lbl); sp.appendChild(strong); return sp; }
  ...
  progBar.className = 'progress-bar '+progClass; progBar.setAttribute('role','progressbar'); progBar.setAttribute('aria-valuenow', String(pct)); progBar.setAttribute('aria-valuemin','0'); progBar.setAttribute('aria-valuemax','100'); progBar.style.setProperty('--progress-width', pct + '%');
        prog.appendChild(progBar); left.appendChild(prog);
  var pctNode = document.createElement('div'); pctNode.className = 'note-muted'; pctNode.textContent = pct + '%'; left.appendChild(pctNode);
        body.appendChild(left);

  // right column (small table)
  var right = document.createElement('div'); right.className = 'w-220';
  var table = document.createElement('table'); table.className = 'table table-condensed';
        var tb = document.createElement('tbody');
  function mkRow(k,v){ var r=document.createElement('tr'); var th=document.createElement('th'); if (k==='Queued') th.className='col-queued'; th.textContent = k; var td=document.createElement('td'); td.textContent = String(v); r.appendChild(th); r.appendChild(td); return r; }
  ...
  var sel = document.createElement('select'); sel.id='fetch-stats-interval'; sel.className='input-sm form-control w-120';
        ...
        // thresholds footer
  var thr = document.createElement('div'); thr.className='small-muted small-help'; thr.textContent = 'Thresholds: warn=' + q_warn + '  crit=' + q_crit + '  dropped_warn=' + d_warn; body.appendChild(thr);
      }
    });
  ...
  // (status-summary card removed; fetch stats now live in the Fetch Queue tab)
      ...
      // DEPRECATION: renderSparkline is a small inline sparkline renderer used
      // in a few places; mark for review and emit a runtime warning if used so
      // we capture usage before removing.
      function renderSparkline(svgId, series, color) {
        try { if (!window._deprecated_warned_renderSparkline) { console.warn('DEPRECATION: renderSparkline() is flagged for removal in future refactors'); window._deprecated_warned_renderSparkline = true; } } catch(e){}
  ...
  if (trTab) { if (caps.traceroute) trTab.parentElement.classList.remove('d-none'); else trTab.parentElement.classList.add('d-none'); }
  var adminTabLink = document.getElementById('tab-admin-link');
  if (adminTabLink) { if (caps.show_admin_link) adminTabLink.classList.remove('d-none'); else adminTabLink.classList.add('d-none'); }
      } catch(e){}
      ...
      if (status.olsr2_on) {
            ...
            fetch('/olsr2', {cache: 'no-store'})
              ...
              .then(function(t) {
                ...
                // If /olsr2 fetch failed, still ensure UI is updated with status data
                updateUI(data);
              });
          } else {
            updateUI(data);
            try {
              // OLSR Links
                ...
                var linkTab = document.querySelector('#mainTabs a[href="#tab-olsr"]');
                if (linkTab) { if (window._uiDebug) console.debug('Showing OLSR tab with links from /status'); linkTab.parentElement.classList.remove('d-none'); }
                ...
                var linkTab = document.querySelector('#mainTabs a[href="#tab-olsr"]');
                if (linkTab) { if (window._uiDebug) console.debug('Keeping OLSR tab visible (no links yet)'); linkTab.parentElement.classList.remove('d-none'); }
              }
        ...
        try { refreshConnBtn.disabled = true; } catch(e){}
          ...
          try {
            if (status.trace_to_uplink && Array.isArray(status.trace_to_uplink) && status.trace_to_uplink.length) {
              var trTab = document.querySelector('#mainTabs a[href="#tab-traceroute"]');
              if (trTab) trTab.parentElement.classList.remove('d-none');
...
window.addEventListener('load', function(){
  ...
  if (window._uiDebug) console.debug('Received /olsr/links', o && (o.links? o.links.length : 'no links'));
      ...
      // Also fetch OLSR2 links
  fetch('/olsr2/links',{cache:'no-store'}).then(function(r){return r.json();}).then(function(o){
  if (window._uiDebug) console.debug('Received /olsr2/links', o);
    try {
      var links = [];
      if (Array.isArray(o)) links = o;
      else if (o && Array.isArray(o.links)) links = normalizeOlsr2Links(o.links);
      else links = normalizeOlsr2Links(o);
      if (links && links.length) { populateOlsr2LinksTable(links); }
    } catch (e) { if (window._uiDebug) console.debug('Error normalizing /olsr2/links', e); }
  }).catch(function(e){ if (window._uiDebug) console.debug('Failed to load /olsr2/links', e); });
  ...
  if (statusEl) statusEl.textContent = 'Refreshing…';
      ...
      fetch('/nodedb/refresh',{cache:'no-store'}).then(function(r){ return r.json(); }).then(function(res){
        ...
        if (o.links && o.links.length) { 
          window._olsr_links = o.links.slice();
          populateOlsrLinksTable(o.links); 
        }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log('Active pane:', targetPane.id, 'classes:', targetPane.className);
  ...
  if (initialActive) {
    ...
    if (initialPane && initialPane.classList.contains('d-none')) {
  ...
  // Wire links_live controls (enable + interval)
  try {
    function renderLinksLiveControls() {
      var chk = document.getElementById('links-live-enable');
      var inp = document.getElementById('links-live-interval');
      if (!chk || !inp) return;
      try {
        var enabled = localStorage.getItem('links_live_enabled');
        var iv = localStorage.getItem('links_live_interval_s');
        if (enabled === null) enabled = '1';
        if (!iv) iv = String(Math.max(1, Math.floor(window._links_poll_interval_ms/1000) || 10));
        chk.checked = (String(enabled) === '1' || String(enabled) === 'true');
        inp.value = String(iv);
      } catch(e) { chk.checked = true; inp.value = String(Math.max(1, Math.floor(window._links_poll_interval_ms/1000) || 10)); }

      // change handlers
      chk.addEventListener('change', function(){
        try { localStorage.setItem('links_live_enabled', chk.checked ? '1' : '0'); } catch(e) {}
        try {
          if (chk.checked) startLinksPoll(); else stopLinksPoll();
        } catch(e) {}
      });
      inp.addEventListener('change', function(){
        try {
          var v = parseInt(inp.value,10);
          if (isNaN(v) || v < 1) v = 1;
          localStorage.setItem('links_live_interval_s', String(v));
          window._links_poll_interval_ms = v * 1000;
          // restart if running
          try { if (window._links_poll_handle) { stopLinksPoll(); startLinksPoll(); } } catch(e) {}
        } catch(e) {}
      });
    }
    try { renderLinksLiveControls(); } catch(e) {}
  } catch(e) {}

  // Clean up polling on unload
  window.addEventListener('beforeunload', function(){ try { stopLinksPoll(); stopLinksPoll(); } catch(e) {} });
    ...
    try {
      ...
      if (id === '#tab-contact') {
        ...
        try {
          ...
          if (contactPane) {
            contactPane.classList.remove('d-none');
          }
        } catch(e) {}
      }
      if (id === '#tab-status') {
        ...
        try {
          ...
          // links_live continuous polling removed: we perform a one-off refresh
          // when the user opens the OLSR Links tab instead.
      ...
      if (id === '#tab-olsr') {
        ...
        try {
          // Add or update OLSR2 links section. This is idempotent and will reuse
          // an existing container/table to avoid duplicating headers/tables when
          // the tab is shown repeatedly.
          function addOlsr2Links(st) {
            try {
              if (!st || !st.olsr2_on) return;
              var tab = document.getElementById('tab-olsr');
              if (!tab) return;
              // Reuse a dedicated container if present
              var container = tab.querySelector('.olsr2-container');
              if (!container) {
                container = document.createElement('div');
                container.className = 'olsr2-container mt-3';

                var h3 = document.createElement('h3');
                h3.className = 'olsr2-header';
                h3.textContent = 'OLSR2 Links';
                container.appendChild(h3);

                var table = document.createElement('table');
                table.className = 'table table-striped table-hover olsr2-table';
                // Canonical id for the OLSR2 table inside the legacy OLSR tab
                // This avoids conflicts with the separate OLSR2 tab table (#olsr2LinksTable)
                table.id = 'tab-olsr-olsr2-table';

                var thead = document.createElement('thead');
                var tr = document.createElement('tr');
                ['Interface', 'Local IP', 'Remote IP', 'Remote Host', 'LQ', 'NLQ', 'Cost', 'Routes', 'Nodes', 'Default'].forEach(function(header){
              ...
              // Populate (or refresh) the tbody with latest OLSR2 links
              var tbody = container.querySelector('table tbody');
              if (!tbody) return;
              // Clear any existing rows
              try { clearChildren(tbody); } catch(e) {}

              fetch('/olsr2/links', {cache:'no-store'})
                .then(function(r){ return r.json(); })
                .then(function(olsr2data){
                  try {
                    if (olsr2data && Array.isArray(olsr2data.links)) {
                      olsr2data.links.forEach(function(link){
                        var tr = document.createElement('tr');
                        ['intf', 'local', 'remote', 'remote_host', 'lq', 'nlq', 'cost', 'routes', 'nodes', 'is_default'].forEach(function(key){
                          var td = document.createElement('td');
            ...
            } catch(e) { /* swallow to avoid breaking tab rendering */ }
          }
          ...
          if (needFetch) {
            ...
            safeFetchStatus({cache:'no-store'}).then(function(st){
              try {
                ...
                addOlsr2Links(st);
                try {
                  fetch('/status/links_live', {cache:'no-store'}).then(function(r){ if(!r||!r.ok) throw new Error('HTTP '+(r&&r.status)); return r.json(); }).then(function(obj){ try { if (obj && obj.links && Array.isArray(obj.links)) { window._olsr_links = obj.links.slice(); populateOlsrLinksTable(window._olsr_links); } } catch(e){} }).catch(function(){ /* ignore */ });
                } catch(e) {}
            ...
            setTabLoaded('tab-olsr');
            safeFetchStatus({cache:'no-store'}).then(function(st2){ addOlsr2Links(st2); }).catch(function(){});
            try {
              fetch('/status/links_live', {cache:'no-store'}).then(function(r){ if(!r||!r.ok) throw new Error('HTTP '+(r&&r.status)); return r.json(); }).then(function(obj){ try { if (obj && obj.links && Array.isArray(obj.links)) { window._olsr_links = obj.links.slice(); populateOlsrLinksTable(window._olsr_links); } } catch(e){} }).catch(function(){ /* ignore */ });
            } catch(e) {}
          }
            ...
            try {
              renderCard({
                ...
                render: function(body, headerRight) {
                  ...
                  var top = document.createElement('div'); top.className = 'flex-gap-8';
                  ...
                  // Fetch queue section
                  if (s.fetch_stats) {
                    var sep = document.createElement('div'); sep.className = 'sep-title'; sep.textContent = 'Fetch Queue'; body.appendChild(sep);
                    var qwrap = document.createElement('div'); qwrap.className = 'mt-8';
...
function renderLogPage() {
  ...
  tbody.innerHTML = '';
    if (!slice.length) {
  var tr = document.createElement('tr'); var td = document.createElement('td'); td.colSpan = 3; td.className = 'text-muted td-empty'; td.textContent = '(no log lines)'; tr.appendChild(td); tbody.appendChild(tr);
  } else {
    slice.forEach(function(ln){ var p = parseLogLine(ln); var tr = document.createElement('tr'); var tdTs = document.createElement('td'); tdTs.className = 'log-ts'; tdTs.textContent = p.ts || ''; var tdLevel = document.createElement('td'); tdLevel.className = 'log-level'; tdLevel.textContent = p.level || ''; if (p.level==='ERROR' || p.level==='ERR' || p.level==='CRITICAL') tdLevel.classList.add('text-danger'); else if (p.level==='WARN' || p.level==='WARNING') tdLevel.classList.add('text-warning'); else if (p.level==='INFO') tdLevel.classList.add('text-info'); else if (p.level==='DEBUG') tdLevel.classList.add('text-muted'); var tdMsg = document.createElement('td'); tdMsg.className = 'log-msg'; tdMsg.textContent = p.msg || ''; tr.appendChild(tdTs); tr.appendChild(tdLevel); tr.appendChild(tdMsg); tbody.appendChild(tr); });
  }
  // pager html
  if (pagerWrap) {
    pagerWrap.innerHTML = '';
    var info = document.createElement('div'); info.textContent = 'Showing '+(start+1)+'-'+(end)+' of '+total+' entries';
  var controls = document.createElement('div'); controls.className = 'log-controls';
  ...
  var span = document.createElement('span'); span.className = 'page-span'; span.textContent = 'Page '+(cache.page+1)+'/'+pages;
  ...
  try { var pane = document.getElementById('tab-log'); if (pane) { pane.classList.remove('hidden'); pane.classList.remove('d-none'); } } catch(e){}
}
...
function enableVirtualLogs() {
  ...
  var th1 = document.createElement('th'); th1.className = 'col-timestamp'; th1.textContent = 'Timestamp';
  var th2 = document.createElement('th'); th2.className = 'col-level'; th2.textContent = 'Level';
  ...
  tbody.classList.add('pos-relative');
        table.appendChild(tbody);

        // spacer to emulate full height
        var spacer = document.createElement('div');
        spacer.id = 'log-virtual-spacer';
  spacer.className = 'spacer'; spacer.style.setProperty('--spacer-height','0px');
  ...
  try {
    renderCard({
      ...
      render: function(body) {
        ...
        table.className = 'table table-condensed';
        table.id = 'log-table';
  var thead = document.createElement('thead'); var trh=document.createElement('tr'); var thh1=document.createElement('th'); thh1.className='col-timestamp'; thh1.textContent='Timestamp'; var thh2=document.createElement('th'); thh2.className='col-level'; thh2.textContent='Level'; var thh3=document.createElement('th'); thh3.textContent='Message'; trh.appendChild(thh1); trh.appendChild(thh2); trh.appendChild(thh3); thead.appendChild(trh);
  var tbodyInner = document.createElement('tbody'); tbodyInner.id = 'log-tbody'; var tr = document.createElement('tr'); var tdEmpty = document.createElement('td'); tdEmpty.colSpan = 3; tdEmpty.className = 'text-muted td-empty'; tdEmpty.textContent = '(no log lines)'; tr.appendChild(tdEmpty); tbodyInner.appendChild(tr);
...
function virtualRender() {
  ...
  spacer.style.setProperty('--spacer-height', (total * rowHeight) + 'px');
  ...
  // position tbody
  tbody.style.setProperty('--translateY', (firstIdx * rowHeight) + 'px'); tbody.classList.add('translate-y');
  // render visible rows
  tbody.innerHTML = '';
    ...
    var tr = document.createElement('tr');
    var tdTs = document.createElement('td'); tdTs.className='log-ts'; tdTs.textContent = p.ts || '';
    var tdLevel = document.createElement('td'); tdLevel.className='log-level'; tdLevel.textContent = p.level || '';
    if (p.level==='ERROR' || p.level==='ERR' || p.level==='CRITICAL') tdLevel.classList.add('text-danger'); else if (p.level==='WARN' || p.level==='WARNING') tdLevel.classList.add('text-warning'); else if (p.level==='INFO') tdLevel.classList.add('text-info'); else if (p.level==='DEBUG') tdLevel.classList.add('text-muted');
    var tdMsg = document.createElement('td'); tdMsg.className='log-msg'; tdMsg.textContent = p.msg || '';
...
function enableTableSorting() {
  ...
  th.classList.add('clickable');
        ...
        th.addEventListener('click', function(){
          ...
          if (!mark) { mark = document.createElement('span'); mark.className='sort-mark sort-mark-gap'; th.appendChild(mark); }
...
function setLoginLink(url, port) {
  ...
  if (!nav || !link) return;
  if (!url) { nav.classList.add('d-none'); return; }
...
function renderConnectionsTable(c, nodedb) {
  ...
  headers.forEach(function(h){ h.classList.add('clickable'); h.onclick = function(){ sortTableByColumn(h.getAttribute('data-key')); }; });
  // Ensure the connections tab pane is visible after rendering (fix intermittent hidden-tab issue)
  try { var pane = document.getElementById('tab-connections'); if (pane) { pane.classList.remove('hidden'); pane.classList.remove('d-none'); } } catch(e){}
}
...
function renderVersionsPanel(v) {
  ...
  headerBody.className = 'panel-body header-row';
  ...
  var titleDiv = document.createElement('div'); titleDiv.className = 'panel-title'; titleDiv.textContent = (v.hostname || 'System');
  var smallDiv = document.createElement('div'); smallDiv.className = 'panel-subtitle'; smallDiv.textContent = smallInfo;
  ...
  function createOlsrStatusBadge(binaryName, exists, running) {
    ...
    icon.className = 'status-olsr-icon';
    var label = document.createElement('span');
    label.textContent = binaryName;
    if (!exists) {
      icon.classList.add('status-olsr-icon--missing');
      badge.classList.add('status-olsr-badge--missing');
      icon.textContent = '✖';
    } else if (exists && running) {
      icon.classList.add('status-olsr-icon--running');
      badge.classList.add('status-olsr-badge--running');
      icon.textContent = '✓';
    } else {
      icon.classList.add('status-olsr-icon--stopped');
      badge.classList.add('status-olsr-badge--stopped');
      icon.textContent = '✖';
    }
  ...
  rawDataSection.classList.add('mt-20');
  ...
  // Styling for versions container moved to CSS file for maintainability
  ...
  itemDiv.className = 'info-item-compact';
  var left = document.createElement('div'); left.className = 'info-label';
  var li = document.createElement('i'); li.className = 'glyphicon glyphicon-' + item.icon; left.appendChild(li); left.appendChild(document.createTextNode(' ' + item.label));
  var valDiv = document.createElement('div'); valDiv.className = 'info-value'; valDiv.textContent = item.value;
  itemDiv.appendChild(left); itemDiv.appendChild(valDiv);
...
function populateTracerouteTable(tracerouteData) {
  ...
  // use global td(val, allowHtml) helper
    ...
    // Make hostname FQDN if it's a short nodename
    if (hostname && hostname.indexOf('.') === -1 && hostname !== '*') {
      hostname += '.wien.funkfeuer.at';
    }
  ...
  try { var pane = document.getElementById('tab-traceroute'); if (pane) { pane.classList.remove('hidden'); pane.classList.remove('d-none'); } } catch(e){}
...
function runTraceroute(){
  ...
  summaryEl.classList.add('small-help');
      hostEl.parentNode.insertBefore(summaryEl, hostEl.nextSibling);
    }
  }
  if (summaryEl) summaryEl.textContent = 'Traceroute: running ...';
  if (pre) { pre.classList.remove('d-none'); pre.classList.add('d-block'); }
  if (pre) pre.textContent='Running traceroute...';
  fetch('/traceroute?target='+encodeURIComponent(target)+'&format=json',{cache:'no-store'}).then(function(r){ return r.json(); }).then(function(data){
    if (pre) pre.textContent = 'JSON response received';
    try { if (window._uiDebug) console.debug('traceroute JSON response:', data); } catch(e){}
      // Parse JSON response with resolved hostnames
      try {
        if (data && data.hops && Array.isArray(data.hops)) {
          var hops = data.hops.map(function(h){ return { hop: h.hop, ip: h.ip, hostname: h.host, ping: h.ping }; });
          if (hops.length > 0) {
            populateTracerouteTable(hops);
            if (pre) { pre.classList.add('d-none'); pre.classList.remove('d-block'); }
            if (summaryEl) summaryEl.textContent = 'Traceroute to ' + (data.target || target) + ': ' + hops.length + ' hop(s)';
            try { if (window._uiDebug) console.debug('Traceroute parsed', hops.length, 'hops from JSON'); } catch(e){}
          } else {
            if (pre) pre.textContent = 'No hops found in traceroute response';
          }
        } else {
          if (pre) pre.textContent = 'Invalid traceroute response format';
        }
      } catch(e) {
        console.error('Error parsing traceroute JSON:', e);
        if (pre) pre.textContent = 'Error parsing traceroute response: ' + e.message;
      }
    }).catch(function(e){
      console.error('Traceroute fetch error:', e);
      if (pre) pre.textContent = 'Error running traceroute: ' + e.message;
      if (summaryEl) summaryEl.textContent = 'Traceroute: error';
    });
}
  ...
  function detectLegacyOlsrd(cb){
    ...
    safeFetchStatus({cache:'no-store'}).then(function(st){
      ...
      try {
        if (st) {
          /*
           * Show the legacy OLSR tab when the backend indicates olsrd is present
           * or when a links array is provided. Previously this check suppressed
           * the legacy tab when olsr2 was running which hid useful legacy
           * data on systems that run both daemons.
           */
          if (st.olsrd_on || (st.links && Array.isArray(st.links) && st.links.length)) show = true;
        }
      } catch(e) { show = false; }
      ...
      if (!show && linkTab) {
        ...
        try {
          fetch('/olsr/links', {cache:'no-store'}).then(function(r){ return r.json(); }).then(function(res){
            ...
            if (show) linkTab.parentElement.classList.remove('d-none'); else linkTab.parentElement.classList.add('d-none');
            if (cb) cb(show);
          }).catch(function(){
...
;(function(){
  ...
  rows.forEach(function(r){ var txt = normalizeText(r.textContent || ''); if (q === '' || txt.indexOf(q) !== -1) { r.classList.remove('d-none'); } else { r.classList.add('d-none'); } });
  }
  ...
  var mark = th.querySelector('.sort-mark'); if (!mark) { mark = document.createElement('span'); mark.className='sort-mark sort-mark-gap'; mark.textContent='↕'; th.appendChild(mark); }
  ...
  if (!th.querySelector('.sort-mark')) { var sm = document.createElement('span'); sm.className='sort-mark sort-mark-gap'; sm.textContent='↕'; th.appendChild(sm); }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  overlays.forEach(function(o){ o.addEventListener('click', function(e){ if (e.target === o) { try { o.setAttribute('aria-hidden','true'); o.classList.add('hidden'); } catch(e){} } }); });
});
