          ...
          fetch('/diagnostics.json', {cache:'no-store'}).then(function(res){ return res.text(); }).then(function(txt){
            ...
            function addGroup(title, obj, groupKey) {
              ...
              Object.keys(obj).sort().forEach(function(k){
                ...
                var descText = '';
                if (meta && meta[k]) {
                  try { var m = meta[k]; if (m.label) displayKey = m.label; if (m.desc) descText = m.desc; }
                  catch(e){}
                }
                td1.textContent = displayKey;
                var td2 = document.createElement('td');
                try {
                  if (typeof obj[k] === 'object') td2.textContent = JSON.stringify(obj[k], null, 2);
                  else td2.textContent = String(obj[k]);
                } catch(e){ td2.textContent = String(obj[k]); }
                tr.appendChild(td1); tr.appendChild(td2); tbody.appendChild(tr);
                if (descText) {
                  var trd = document.createElement('tr'); var dtd = document.createElement('td'); dtd.colSpan=2; dtd.style.fontSize='12px'; dtd.style.color='#666'; dtd.textContent = descText; trd.appendChild(dtd); tbody.appendChild(trd);
                }
              });
